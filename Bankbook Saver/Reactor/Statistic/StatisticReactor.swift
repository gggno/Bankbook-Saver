...
class StatisticReactor: Reactor {
    ...
    enum Action {
        ...
        case updateSegmentIndexAction(segmentIndex: Int)      // 세그먼트 컨트롤 인덱스 가져오기
        case segmentControlIndexResetAction     // 세크먼트 컨트롤 인덱스 2로 초기화
        case moveToDateAction(selectedDateCount: Int)
    }
    
    enum Mutation {
        case fetchDbDataMutation(dbDatas: [HomeDataEntity])
        case updateSegmentIndexMutation(segmentIndex: Int)
        case segmentControlIndexResetMutation
        case selectedDateCountResetMutation       // 선택한 날짜 카운트 0으로 초기화
        case updateDisplayDatasMutation(statisticData: StatisticData)
        case updateSelectedDateCountMutation(selectedDateCount: Int)
    }
    ...
    struct State {
        ...
        var lastCompareText: String = ""               // 지난달과 지출 비교
...
extension StatisticReactor {
    func mutate(action: Action) -> Observable<Mutation> {
        ...
        case .fetchDbDataAction:
            ...
            return .just(.fetchDbDataMutation(dbDatas: allDbDatas))
            
        case .updateSegmentIndexAction(let selectedIndex):
            print("updateSegmentIndexAction")
            
            let statisticData = getStatisticData(selectedIndex: selectedIndex)
            
            return Observable.concat([
                .just(.updateSegmentIndexMutation(segmentIndex: selectedIndex)),
                .just(.updateDisplayDatasMutation(statisticData: statisticData)),
        ...
        case .moveToDateAction(let count):
            ...
            let statisticData = getStatisticDataForMoveDate(count: count)
            
            return Observable.concat([
                .just(.updateDisplayDatasMutation(statisticData: statisticData)),
                .just(.updateSelectedDateCountMutation(selectedDateCount: count))
            ])
        }
    }
}
...
extension StatisticReactor {
    func reduce(state: State, mutation: Mutation) -> State {
        ...
        case .updateDisplayDatasMutation(let statisticData):
            
            newState.dateText = statisticData.dateText
            newState.outComeMoneyText = statisticData.outComeMoneyText
            newState.inComeMoneyText = statisticData.inComeMoneyText
            newState.lastCompareText = statisticData.lastCompareText
            newState.barChartDatas = statisticData.barChartDatas
            newState.pieChartDatas = statisticData.pieChartDatas
            newState.inOutDatas = statisticData.inOutDatas
...
extension StatisticReactor {
    // StatisticData 가져오기(일별, 주별, 월별을 눌렀을 때 활용)
    private func getStatisticData(selectedIndex: Int) -> StatisticData {
        let today = Date()
        ...
        let updateSegmentType = DateRangeType(rawValue: selectedIndex) ?? .monthly
        
        switch updateSegmentType {
        case .daily: // 일별
            print("case 0: 일별")
            ...
            for data in repeatDatas {
                ...
                if let changedDate = calendar.date(from: dateComponents) {
                    let changedRepeatData = HomeDataEntity(homeData: data, changedDate: changedDate)
            ...
            for data in dayFilterDatas {
                ...
                if inOutDatas[purposeDate] == nil {
                    inOutDatas[purposeDate] = [InOutCellInfo(id: data._id.stringValue, transactionType: data.transactionType, emoji: emoji, money: money, detailUse: detailUse, category: category)]
                } else {
                    inOutDatas[purposeDate]! += [InOutCellInfo(id: data._id.stringValue, transactionType: data.transactionType, emoji: emoji, money: money, detailUse: detailUse, category: category)]
                }
            }
        ...
        let statisticData = StatisticData(dateText: dateText, outComeMoneyText: outComeMoneyText, inComeMoneyText: inComeMoneyText, lastCompareText: lastCompareText, barChartDatas: barChartDatas, pieChartDatas: pieChartDatas, inOutDatas: inOutDatas)
        
        return statisticData
    }
    
    // StatisticData 가져오기(일별, 주별, 월별 날짜를 좌우로 변경할 때)
    private func getStatisticDataForMoveDate(count: Int) -> StatisticData {
        let calendar = Calendar.current
        let today = Date()
        var selectedDate = Date()
        
        let moveSegmentType = DateRangeType(rawValue: currentState.selectedIndex) ?? .monthly
        
        switch moveSegmentType {
        case .daily:
